...
int main()
{
    /********  static_cast  *********/
    /**
        1. 强制类型转换，从量上直接从大类型转换到小类型和从小类型转换到大类型都可以, 
          但是指针从大类型到小类型或者小类型到大类型都不可以编译, 但是C的强制转换
          会编译成功,进而导致莫名其妙的错误结果,也就是说static_cast会进行安全类型检查
            error:  other pointer <---> other pointer
        2. 找回void *指针中存在的值, 也就是从 void*  转换成其他类型   
            right:  void* <--> pointer
    */
    ...
    void* p2 = static_cast<void*>(dp);

    // bad example
    int* ptr = &b;
    // error: invalid static_cast from type 'void*' to type 'int*'
    // can't compile in C++
    // double *ip = static_cast<double *>(ptr); 
    ...
    */
    ...
    // std::cout << hex << *pnum << std::endl;
    //std::cout << *psnum << endl;
    std::cout << *ps2num << std::endl;
    std::cout << ps2num << std::endl;
    //std::cout << ps3num << std::endl;
